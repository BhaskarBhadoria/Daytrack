...
export default function Attendance() {
  ...
  const [records, setRecords] = useState({}); // subject_id -> status for selected date
  ...
  async function loadAll() {
    setLoading(true);
    try {
      const [subs, sum] = await Promise.all([api.getSubjects(), api.getAttendanceSummary()]);
      setSubjects(subs);
      setSummary(sum);
      await loadRecords(date, subs);
    } catch (err) {
  ...
  return (
    <div className="attendance-page">
      ...
      {subjects.length === 0 ? (
        <p className="empty">Add your subjects above to start tracking attendance.</p>
      ) : (
        <>
